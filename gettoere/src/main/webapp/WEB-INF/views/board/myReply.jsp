...
<section class="content">
	<div class='box'>
		<div class="box-header with-border">
				<h3 class="box-title">내가 쓴 댓글 목록</h3>
		</div>
	
		<div class='box-body'>
			<select name="searchType">
				<option value="ntc" <c:out value="${cri.searchType eq 'ntc'?'selected':''}"/>>전체</option>
				<option value="n" <c:out value="${cri.searchType eq 'n'?'selected':''}"/>>게시판이름</option>
				<option value="t" <c:out value="${cri.searchType eq 't'?'selected':''}"/>>제목</option>
				<option value="c" <c:out value="${cri.searchType eq 'c'?'selected':''}"/>>댓글내용</option>
				<option value="nt" <c:out value="${cri.searchType eq 'nt'?'selected':''}"/>>게시판이름+제목</option>
				<option value="tc" <c:out value="${cri.searchType eq 'tc'?'selected':''}"/>>제목+댓글내용</option>
		...
		<table class = "table table-bordered">
			...
			<c:forEach items="${list}" var ="listVO">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${fn:length(listVO.contents) > 10}">
								<c:set var="cont" value="${fn:substring(listVO.contents, 0 , 9)}"></c:set>
								${cont}...
							</c:when>
							<c:otherwise>${listVO.contents}</c:otherwise>
						</c:choose>
					</td>
		...
		<div class = "text-center">
			<ul class = "pagination">
				<c:if test="${pageMaker.isprev}">
					<li><a href = "myReply${pageMaker.listMakeSearch(pageMaker.startPage-1)}">&laquo;</a></li>
				</c:if>
			
				<c:forEach begin = "${pageMaker.startPage}" end="${pageMaker.endPage}" var = "idx">
			
					<li <c:out value="${pageMaker.cri.page == idx ? 'class = active':''}"/>>
						<a href ="myReply${pageMaker.listMakeSearch(idx)}">${idx}</a>
					</li>
				</c:forEach>
				<c:if test="${ pageMaker.isnext && pageMaker.endPage >0}">
					<li><a href = "myReply${pageMaker.listMakeSearch(pageMaker.endPage+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	</div>
...
<script>
	//상단 제목 변경. ssh. 171129
	$(".content-header h1").html("내가 쓴 댓글 목록");
