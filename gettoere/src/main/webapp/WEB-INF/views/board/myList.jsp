...
<section class="content">
	<div class='box'>
		<div class="box-header with-border">
				<h3 class="box-title">내가 쓴 글 목록</h3>
		</div>
		...
		<div class = "text-center">
			<ul class = "pagination">
				<c:if test="${pageMaker.isprev}">
					<li><a href = "myList${pageMaker.listMakeSearch(pageMaker.startPage-1)}">&laquo;</a></li>
				</c:if>
			
				<c:forEach begin = "${pageMaker.startPage}" end="${pageMaker.endPage}" var = "idx">
			
					<li <c:out value="${pageMaker.cri.page == idx ? 'class = active':''}"/>>
						<a href ="myList${pageMaker.listMakeSearch(idx)}">${idx}</a>
					</li>
				</c:forEach>
				<c:if test="${ pageMaker.isnext && pageMaker.endPage >0}">
					<li><a href = "myList${pageMaker.listMakeSearch(pageMaker.endPage+1)}">&raquo;</a></li>
				</c:if>
			</ul>
		</div>
	</div>
...
<script>
	//상단 제목 변경. ssh. 171129
	$(".content-header h1").html("내가 쓴 글 목록");
